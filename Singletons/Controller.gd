...
func getMove():
	...
	joyMove.z = Input.get_joy_axis(joyPort,joyMoveV)
	move = Vector3(0,0,0)
	...
	if(Input.is_key_pressed(pressLeft)):
		...
		move.x = 1.0
	if((abs(joyMove.x) + abs(joyMove.z)) > joyMoveDz):
		return joyMove
...
func getCursorAngle(position, mouse):
	...
	if((abs(joyCursor.x) + abs(joyCursor.y)) > joyCursorDz):
		...
		if(mouseHasMoved):
			var cursorVec = Vector2(position - mouse)
			cursorVec.x = -cursorVec.x
			cursorAngle = cursorVec.angle()
	return Vector3(0,cursorAngle,0)
